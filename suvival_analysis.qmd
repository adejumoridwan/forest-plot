...
fir_rad_data <- 
  cancer_data |>
  mutate(
    first_visit_to_radiotheraphy = round(difftime(CHEMOTHERAPY_FIRST_DATE
...
fir_rad_data <- 
  cancer_data |>
  mutate(
    first_visit_to_radiotheraphy = round(
...
fir_rad_data <- 
  cancer_data |>
  mutate(
    first_visit_to_radiotheraphy = round(difftime(RADIOTHERAPY_FIRST_DATE, SURGERY_DATE, units = "weeks")),
    AGE = floor(time_length(interval(DOB, RADIOTHERAPY_FIRST_DATE), unit = "year")),
    AGE_GROUP = case_when(
      AGE <= 40 ~ "Less than 40",
      TRUE ~ "Greater than 40"
